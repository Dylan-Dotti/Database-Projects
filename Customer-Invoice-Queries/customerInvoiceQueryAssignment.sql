...
-- Note that soe queries may result in no output. This is expected and is the intention of filtering.
...
JOIN CUSTOMER
ON INVOICE.CUS_CODE = CUSTOMER.CUS_CODE
AND CUSTOMER.CUS_FNAME = 'Kathy'
AND CUSTOMER.CUS_LNAME = 'Smith';

-- 5. Use ALTER and UPDATE commands to add a column called ISO9000 to Vendor. This column should hold a single character and should be 'N' by default and should never be null. 
...
SELECT DISTINCT V_NAME, P_DESCRIPT
FROM VENDOR
...
SELECT CUSTOMER.CUS_FNAME, CUSTOMER.CUS_LNAME, SUM(LINE.LINE_UNITS * LINE.LINE_PRICE) as total
FROM CUSTOMER
JOIN INVOICE
ON INVOICE.CUS_CODE = CUSTOMER.CUS_CODE
JOIN LINE
